...
public class KSolitaire
{
	...
	public static final Point SHOW_POS = new Point(DECK_POS.x + KCard.CARD_WIDTH + 5, DECK_POS.y);
	public static final Point FINAL_POS = new Point(SHOW_POS.x + KCard.CARD_WIDTH + 150, DECK_POS.y);
	public static final Point PLAY_POS = new Point(DECK_POS.x, FINAL_POS.y + KCard.CARD_HEIGHT + 30);

	// GAMEPLAY STRUCTURES
	private static KFinalStack[] final_cards;// Foundation Stacks
	private static KCardStack[] playCardStack; // Tableau stacks
	private static final KCard newCardPlace = new KCard();// waste card spot
	private static KCardStack deck; // populated with standard 52 card deck
	...
	protected static KCard moveCard(KCard c, int x, int y)
	{
		c.setBounds(new Rectangle(new Point(x, y), new Dimension(KCard.CARD_WIDTH + 10, KCard.CARD_HEIGHT + 10)));
	...
	{
		KSolitaire.score += deltaScore;
		String newScore = "Score: " + KSolitaire.score;
	...
	protected static void updateTimer()
	{
		...
		if (KSolitaire.time % 10 == 0)
		{
			setScore(-2);
		}
		String time = "Seconds: " + KSolitaire.time;
	...
	{
		private KCard prevCard = null;// tracking card for waste stack
		private KCard movedCard = null;// card moved from waste stack
		...
		private KCard card = null; // card to be moved
		// used for moving single cards
		private KCardStack source = null;
		private KCardStack dest = null;
		// used for moving a stack of cards
		private KCardStack transferStack = new KCardStack(false);

		private boolean validPlayStackMove(KCard source, KCard dest)
		{
			int s_val = source.getValue().ordinal();
			int d_val = dest.getValue().ordinal();
			KCard.Suit s_suit = source.getSuit();
			KCard.Suit d_suit = dest.getSuit();
			...
			if ((s_val + 1) == d_val)
			{
				...
				{
				case SPADES:
					if (d_suit != KCard.Suit.HEARTS && d_suit != KCard.Suit.DIAMONDS)
						return false;
					else
						return true;
				case CLUBS:
					if (d_suit != KCard.Suit.HEARTS && d_suit != KCard.Suit.DIAMONDS)
						...
						return true;
				case HEARTS:
					if (d_suit != KCard.Suit.SPADES && d_suit != KCard.Suit.CLUBS)
						return false;
					else
						return true;
				case DIAMONDS:
		...
		private boolean validFinalStackMove(KCard source, KCard dest)
		{
			int s_val = source.getValue().ordinal();
			int d_val = dest.getValue().ordinal();
			KCard.Suit s_suit = source.getSuit();
		...
		public void mousePressed(MouseEvent e)
		{
			...
			if (newCardButton.contains(start) && deck.showSize() > 0)
			{
				...
				table.add(KSolitaire.moveCard(c, SHOW_POS.x, SHOW_POS.y));
				c.repaint();
				table.repaint();
				prevCard = c;
			}
			...
			if (movedCard != null)
			{
				...
				for (int x = 0; x < NUM_PLAY_DECKS; x++)
				{
					...
					if (dest.empty() && movedCard != null && dest.contains(stop)
							&& movedCard.getValue() == KCard.Value.KING)
					{
				...
				for (int x = 0; x < NUM_FINAL_DECKS; x++)
				{
					...
					{
						if (movedCard.getValue() == KCard.Value.ACE)
						{
							dest.push(movedCard);
							table.remove(prevCard);
							dest.repaint();
			...
			{ // Moving from PLAY TO PLAY
				for (int x = 0; x < NUM_PLAY_DECKS; x++)
				{
							...
							&& validPlayStackMove(card, dest.getFirst()) && transferStack.showSize() == 1)
					{
						...
						{
							KCard temp = source.getFirst().setFaceup();
							temp.repaint();
							source.repaint();
						}
						...
						validMoveMade = true;
						break;
					} else if (dest.empty() && card.getValue() == KCard.Value.KING && transferStack.showSize() == 1)
					...
					if (dest.empty() && dest.contains(stop) && !transferStack.empty()
							&& transferStack.getFirst().getValue() == KCard.Value.KING)
					{
				...
				for (int x = 0; x < NUM_FINAL_DECKS; x++)
				{
					...
					{// TO EMPTY STACK
						...
						{
							if (card.getValue() == KCard.Value.ACE)
							{
								KCard c = source.popFirst();
								c.repaint();
								if (source.getFirst() != null)
								{
	...
	private static void playNewGame()
	{
		...
		final_cards = new KFinalStack[NUM_FINAL_DECKS];
		for (int x = 0; x < NUM_FINAL_DECKS; x++)
		{
			final_cards[x] = new KFinalStack();

			final_cards[x].setXY((FINAL_POS.x + (x * KCard.CARD_WIDTH)) + 10, FINAL_POS.y);
		...
		playCardStack = new KCardStack[NUM_PLAY_DECKS];
		for (int x = 0; x < NUM_PLAY_DECKS; x++)
		{
			playCardStack[x] = new KCardStack(false);
			playCardStack[x].setXY((DECK_POS.x + (x * (KCard.CARD_WIDTH + 10))), PLAY_POS.y);
